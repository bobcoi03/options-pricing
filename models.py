...
def black_scholes_option_price(spot_price, strike_price, risk_free_rate, t, sigma, dividend_yield, option_type):
    d1 = (math.log(spot_price / strike_price) + (risk_free_rate - dividend_yield + (sigma ** 2) / 2) * t) / (sigma * math.sqrt(t))
    d2 = d1 - sigma * math.sqrt(t)

    if option_type == 'call':
        option_price = (spot_price * math.exp(-dividend_yield * t) * norm.cdf(d1)) - (strike_price * math.exp(-risk_free_rate * t) * norm.cdf(d2))
    elif option_type == 'put':
        option_price = (strike_price * math.exp(-risk_free_rate * t) * norm.cdf(-d2)) - (spot_price * math.exp(-dividend_yield * t) * norm.cdf(-d1))
    else:
